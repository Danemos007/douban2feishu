...
## 开发红线
1. **关于逻辑缺陷**： 严禁编写依赖硬编码字符串或模糊数字等不可靠数据来进行判断的脆弱逻辑。
2. **关于网络层安全**： 必须确保核心层（如网络请求）的类型绝对安全，并验证库与类型定义的版本一致性。
3. **关于外部API**： 所有与外部系统（如API）交互的数据边界，都必须进行严格的运行时校验，绝不能仅凭假设。
4. **关于代码整洁**： 代码实现必须逻辑清晰、路径唯一，严禁出现多次赋值或条件覆盖等导致维护困难的混乱写法。

---
## 🧠 已知技术债务与核心约束

为了保证项目稳定，我们做出了一些主动的技术妥协。你在工作时必须遵守这些约束：

1.  **[TD-001] Zod 库版本必须维持在 v3**:
    * **背景**: 我们尝试过升级Zod v4，但它与项目的模块系统存在无法快速解决的架构冲突。
    * **你的行动准则**: 在进行任何与Schema、数据验证或依赖库升级相关的任务时，你**必须**将此约束作为最高优先级。禁止在你的方案中引入Zod v4+的任何特性。所有相关的详细信息记录在douban2feishu/docs/的 `TECH_DEBT.md` 文件中。
---
...
### 豆瓣测试账号
- **用户ID**: 290244208
- **Cookie**: bid=JsGI-6ZBE0c; ll="118282"; _pk_ref.100001.4cf6=%5B%22%22%2C%22%22%2C1760080938%2C%22https%3A%2F%2Fwww.google.com%2F%22%5D; _pk_id.100001.4cf6=17824e1d0ae203e3.1760080938.; _pk_ses.100001.4cf6=1; ap_v=0,6.0; dbcl2="290244208:A5IRromm0FU"; ck=huCt; frodotk_db="fc6cf7883dcab7daf546e698b960278e"; push_noty_num=0; push_doumail_num=0
