    ...
    try {
      ...
      if (infoElement.length > 0) {
        ...
        if (releaseDateElement.length > 0) {
          ...
          // 去除地区信息，只保留日期 (如: "2010-12-16(中国大陆)" -> "2010-12-16")
          ...
          if (dateMatch) {
            ...
            if (yearMatch) {
              ...
              movie.releaseDate = dateText; // 兜底保留原始值
            }
          }
        } else {
          // 电视剧可能使用首播日期
          ...
          if (firstAirSpan.length > 0) {
            ...
            if (firstAirText) {
              // 同样处理首播日期，去除地区信息
              const dateMatch = firstAirText.match(/^(\d{4}-\d{2}-\d{2})/);
              if (dateMatch) {
                movie.releaseDate = dateMatch[1];
              } else {
