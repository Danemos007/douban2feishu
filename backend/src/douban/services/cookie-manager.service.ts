...
export class CookieManagerService {
   ...
   * 根据不同域名调整Host头
   */
  getHeadersForDomain(domain: 'book' | 'movie' | 'music' | 'www', cookie?: string): Record<string, string> {
   ...
   * 创建Cookie备份和版本管理
   */
  createCookieBackup(userId: string, cookie: string): {
    timestamp: Date;
    cookieHash: string;
    preview: string;
  } {
    ...
    // 简单的hash实现
    const cookieHash = require('crypto').createHash('sha256').update(cookie).digest('hex').substring(0, 16);
