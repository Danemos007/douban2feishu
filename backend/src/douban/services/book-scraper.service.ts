...
import { HtmlParserService, ParsedUserState, ParsedListPage } from './html-parser.service';
...
export class BookScraperService {
  ...
  async fetchUserBooks(
    ...
    try {
      ...
      this.logger.log(`Successfully fetched ${books.length} complete book records`);
      return books;
   ...
   * 获取单本书的详细信息
   */
  private async fetchBookDetail(bookId: string, cookie: string): Promise<BookData> {
