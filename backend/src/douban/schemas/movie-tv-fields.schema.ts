 ...
 * 严格验证豆瓣影视ID格式
 */
export const MovieSubjectIdSchema = z.string()
...
export const MovieTitleSchema = z.object({
  main: z.string().min(1, '影片名称不能为空').max(500, '影片名称长度不能超过500字符'),
 ...
 * 导演Schema
 */
export const DirectorsSchema = z.array(
  z.object({
 ...
 * 编剧Schema
 */
export const WritersSchema = z.array(
  z.object({
...
export const LanguagesSchema = z.array(
  z.string().min(1, '语言不能为空').max(30, '语言长度不能超过30字符')
).default([]);
 ...
 * 影视类型/分类Schema
 */
export const MovieGenresSchema = z.array(
  z.string().min(1, '类型不能为空').max(20, '类型长度不能超过20字符')
).min(1, '影视作品必须至少有一个类型');
 ...
 */
export const MovieUserRatingSchema = z.number()
  .int('用户评分必须是整数')
  .min(1, '用户评分不能小于1星')
  .max(5, '用户评分不能大于5星')
  .optional();
 ...
 * 用户标签Schema (影视通用)
 */
export const MovieUserTagsSchema = z.array(
  z.string()
    .min(1, '标签不能为空')
 ...
 * 用户状态Schema (影视通用)
 */
export const MovieUserStatusSchema = z.enum(['wish', 'do', 'collect'])
...
export function validateTvSeriesComplete(
  data: unknown
): { success: true; data: TvSeriesComplete } | { success: false; error: string } {
  try {
...
export function validateDocumentaryComplete(
  data: unknown
): { success: true; data: DocumentaryComplete } | { success: false; error: string } {
  try {
...
export function isValidTvSeriesComplete(value: unknown): value is TvSeriesComplete {
  try {
...
export function isValidDocumentaryComplete(value: unknown): value is DocumentaryComplete {
  try {
