...
async function testPiperDuration() {
  console.log('=== 测试《鹬 Piper》片长解析修复 ===');
  
  try {
    const url = 'https://movie.douban.com/subject/26766869/';
    console.log(`[请求] ${url}`);
    ...
    console.log(`方法1 - v:runtime 属性: ${durationElement.length > 0 ? durationElement.text().trim() : '未找到'}`);
    
    // 方法2: 从一般信息中查找片长（新修复的方法）
