...
describe('豆瓣字段映射配置验证测试', () => {
  ...
  describe('辅助函数测试', () => {
    it('getVerifiedFieldMapping应该返回正确的映射', () => {
      expect(getVerifiedFieldMapping('movies')).toBe(VERIFIED_MOVIES_FIELD_MAPPING);
      expect(getVerifiedFieldMapping('books')).toBe(VERIFIED_BOOKS_FIELD_MAPPING);
      expect(getVerifiedFieldMapping('tv')).toBe(VERIFIED_MOVIES_FIELD_MAPPING);
      expect(getVerifiedFieldMapping('documentary')).toBe(VERIFIED_MOVIES_FIELD_MAPPING);
    });
    ...
    it('verifiedDoubanFieldToChineseName应该正确转换', () => {
      expect(verifiedDoubanFieldToChineseName('subjectId', 'movies')).toBe('Subject ID');
      expect(verifiedDoubanFieldToChineseName('coverImage', 'movies')).toBe('封面图');
      expect(verifiedDoubanFieldToChineseName('unknownField', 'movies')).toBe('unknownField');
    });
  ...
  describe('业务逻辑验证', () => {
    ...
    it('验证来源覆盖度应该合理', () => {
      ...
      // 核心文件应该有较高的覆盖度
      expect(stats.sourceCoverage['sync-all-movies-fixed.ts']).toBeGreaterThan(15);
      expect(stats.sourceCoverage['sync-movie-from-cache.ts']).toBeGreaterThan(10);
      expect(stats.sourceCoverage['sync-from-cache.ts']).toBeGreaterThan(10);
      expect(stats.sourceCoverage['real-douban-data-sync.ts']).toBeGreaterThan(10);
    });
  });
