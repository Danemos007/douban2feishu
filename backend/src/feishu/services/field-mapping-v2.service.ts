 ...
 */
@Injectable()
export class FieldMappingV2Service {
  private readonly logger = new Logger(FieldMappingV2Service.name);
    ...
    try {
      ...
      if (fieldsToCreate.length > 0) {
        ...
        try {
          ...
          this.logger.error('Failed to create some fields:', error);
      ...
      this.logger.error('Auto-configure field mappings failed:', error);
      throw error;
    }
  }
  ...
  async previewFieldMappings(
    ...
    try {
      ...
      this.logger.error('Failed to preview field mappings:', error);
      throw error;
    }
  }
