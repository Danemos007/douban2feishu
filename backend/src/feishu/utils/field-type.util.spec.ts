  ...
  describe('isRatingFieldType', () => {
    ...
    describe('边界情况和异常处理', () => {
      ...
      it('应该处理null/undefined fieldName', () => {
        expect(() => isRatingFieldType(null as any, FeishuFieldType.Number)).not.toThrow();
        expect(() => isRatingFieldType(undefined as any, FeishuFieldType.Number)).not.toThrow();
      });
    ...
    describe('类型参数无关性验证', () => {
      ...
      it('相同字段名在不同fieldType下应返回相同结果', () => {
        ...
        const resultWithNumber = isRatingFieldType(fieldName, FeishuFieldType.Number);
        const resultWithRating = isRatingFieldType(fieldName, FeishuFieldType.Rating);  
        const resultWithText = isRatingFieldType(fieldName, FeishuFieldType.Text);
    ...
    describe('性能和稳定性', () => {
      ...
      it('应该能处理长字段名', () => {
        const longFieldName = '这是一个很长很长很长很长很长很长的我的评分字段名'.repeat(10);
        expect(() => isRatingFieldType(longFieldName, FeishuFieldType.Number)).not.toThrow();
