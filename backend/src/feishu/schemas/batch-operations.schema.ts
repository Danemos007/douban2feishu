...
export const FeishuBatchCreateRequestSchema = z.object({
  ...
  ).min(1, '批量创建至少需要一条记录').max(500, '单次批量创建最多500条记录'),
});
...
export const FeishuBatchUpdateRequestSchema = z.object({
  ...
  ).min(1, '批量更新至少需要一条记录').max(500, '单次批量更新最多500条记录'),
});
...
export const FeishuBatchDeleteRequestSchema = z.object({
  records: z.array(z.string().min(1, 'record_id不能为空'))
    .min(1, '批量删除至少需要一条记录')
 ...
 */
export const FeishuBatchOperationResponseSchema = z.object({
  // API 状态
  ...
  data: z.object({
    ...
    // 批量更新响应
    updated_records: z.array(z.object({
      record_id: z.string(),
    })).optional(),
    ...
    // 操作失败的记录
    failed_records: z.array(z.object({
...
export type FeishuBatchCreateRequest = z.infer<typeof FeishuBatchCreateRequestSchema>;
export type FeishuBatchUpdateRequest = z.infer<typeof FeishuBatchUpdateRequestSchema>;
export type FeishuBatchDeleteRequest = z.infer<typeof FeishuBatchDeleteRequestSchema>;
export type FeishuBatchOperationResponse = z.infer<typeof FeishuBatchOperationResponseSchema>;
...
export type BatchOperationProgress = z.infer<typeof BatchOperationProgressSchema>;
