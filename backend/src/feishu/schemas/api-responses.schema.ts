 ...
 * 飞书 API 基础响应 Schema
 */
export const FeishuBaseResponseSchema = z.object({
  code: z.number(),
  msg: z.string(),
}).passthrough();
 ...
 */
export const FeishuErrorResponseSchema = z.object({
  code: z.number().refine((val) => val !== 0, { message: '这不是一个错误响应' }),
  msg: z.string().min(1),
  ...
  rate_limit: z.object({
    ...
    rate_limit_detail: z.array(z.object({
      period: z.string(),
      limit: z.number(),
      consumed: z.number(),
    })).optional(),
 ...
 */
export const FeishuPaginatedResponseSchema = <T extends z.ZodTypeAny>(itemSchema: T) => 
 ...
 */
export const FeishuSingleItemResponseSchema = <T extends z.ZodTypeAny>(itemSchema: T) =>
  FeishuSuccessResponseSchema.extend({
    data: itemSchema,
  });
 ...
 */
export const FeishuRequestContextSchema = z.object({
 ...
 */
export const FeishuResponseMetadataSchema = z.object({
  request_id: z.string(),
 ...
 */
export const FeishuHealthCheckResponseSchema = z.object({
...
export type FeishuOperationResponse = z.infer<typeof FeishuOperationResponseSchema>;
...
export type FeishuResponseMetadata = z.infer<typeof FeishuResponseMetadataSchema>;
export type FeishuHealthCheckResponse = z.infer<typeof FeishuHealthCheckResponseSchema>;

// 泛型类型
export type FeishuPaginatedResponse<T> = z.infer<ReturnType<typeof FeishuPaginatedResponseSchema<z.ZodType<T>>>>;
export type FeishuSingleItemResponse<T> = z.infer<ReturnType<typeof FeishuSingleItemResponseSchema<z.ZodType<T>>>>;
...
export function getErrorMessage(code: number): string {
  return FeishuErrorCodeMap[code as keyof typeof FeishuErrorCodeMap] || `未知错误 (${code})`;
}

/**
 * 辅助函数：判断响应是否成功
 */
export function isSuccessResponse(response: FeishuBaseResponse): response is FeishuSuccessResponse {
