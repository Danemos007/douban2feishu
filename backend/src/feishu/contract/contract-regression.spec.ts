...
describe('飞书API契约回归测试', () => {
  describe('字段查询API契约', () => {
    ...
    it('应验证主键字段的特殊性', () => {
      // Arrange
      const response = FeishuFieldsResponseSchema.parse(fieldsFixture);
      const primaryField = response.data.items.find(
  ...
  describe('Schema向前兼容性验证', () => {
    it('Schema应容忍额外的未知字段', () => {
      ...
      // Act
      const parseResult =
        FeishuFieldsResponseSchema.safeParse(fieldsWithExtraData);
    ...
    it('应拒绝缺少关键字段的响应', () => {
      ...
      // Act
      const parseResult =
        FeishuFieldsResponseSchema.safeParse(incompleteResponse);
