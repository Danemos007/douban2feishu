 ...
 * 飞书记录查询响应Schema
 */
const FeishuRecordsResponseSchema = z.object({
  // API状态：严格验证
  code: z.number().refine((val) => val === 0, { message: "飞书记录查询失败" }),
  msg: z.string(),
 ...
 */
const FeishuRecordCreateRequestSchema = z.object({
  fields: z.record(z.string(), FeishuRecordFieldValueSchema),
}).passthrough();
 ...
 * 飞书记录搜索请求Schema
 */
const FeishuSearchRecordRequestSchema = z.object({
  page_size: z.number().min(1).max(500).optional(),
  page_token: z.string().optional(),
  filter: z.object({
    conditions: z.array(z.object({
      field_id: z.string().min(1),
      operator: z.enum(['is', 'isNot', 'contains', 'doesNotContain', 'isEmpty', 'isNotEmpty']),
...
export type FeishuRecordCreateRequest = z.infer<typeof FeishuRecordCreateRequestSchema>;
export type FeishuSearchRecordRequest = z.infer<typeof FeishuSearchRecordRequestSchema>;
export type FeishuRecordFieldValue = z.infer<typeof FeishuRecordFieldValueSchema>;
